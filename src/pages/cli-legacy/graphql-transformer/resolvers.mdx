...
*Migration/Update Note* In Opensearch versions >= 1.0 you no longer need to use the path /doc. Using /doc in custom resolvers may cause issues.
...
*Migration/Update Note* The path for the ElasticSearch total was `ctx.result.hits.total` and in OpenSearch is now `ctx.result.hits.total.value`
